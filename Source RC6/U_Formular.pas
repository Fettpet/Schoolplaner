...
begin
  ...
  wbk := excel.Workbooks.Open(Ordner_data + '\Vorlagen\Antrag.xls');
  ws := wbk.ActiveSheet;
  ws.name := kuerzel;
  ausfuellen;
end;
...
begin
  ...
  begin
    readln(Ferien, temp);
    //--- Ausnameabfrage für spezielle Tage ------------------------------------
    if temp = 'Pfingsten'
    then  begin
    ...
    else if temp = 'Brückentag' then begin
            anzahl_zusatz := anzahl_zusatz + 1;
            readln(Ferien, temp);
            wochentag := DayOfWeek(StrToDate(temp));
            case wochentag of
            ...
            ws.Cells[23, 4 + spalten_pos] := 'Brückentag ' + temp;
            bruecken_spalte := 4 + spalten_pos;
            bruecken_pos := anzahl_zusatz;
    end
    //--- Reguläre Zusätze am Formularende -------------------------------------
    else  begin
            anzahl_zusatz := anzahl_zusatz + 1;
...
begin
  if fileexists(Ordner_data + '\Excel-Files\Antrag_' + kuerzel + '_' + jahr + '.xls') = true
    then deletefile(Ordner_data + '\Excel-Files\Antrag_' + kuerzel + '_' + jahr + '.xls');
  wbk.SaveAs(Ordner_data + '\Excel-Files\Antrag_' + kuerzel + '_' + jahr + '.xls');
  excel.Visible := true;
end;
